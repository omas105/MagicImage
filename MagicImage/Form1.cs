...
namespace MagicImage
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Shown(object sender, EventArgs e)
        {
            ...
            //byte[] payload = LoadPayload("text.zip");
            //Debug.WriteLine($"payload size {payload.GetLongLength(0)} bytes");
            //pbOrg.Image = new Bitmap(@"c:\emma.jpg");
            //Bitmap image = MakeImage(@"c:\emma.jpg", payload);
            //pbResult.Image = image;
            //image.Save("result.bmp", ImageFormat.Bmp);
            //payload = GetSecret(image);
            //SavePayload(payload, "result.txt");
            //image.Save($"image {DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss")}.png", ImageFormat.Png);
        }
                ...
                if (e.Data.GetDataPresent(DataFormats.FileDrop))
                {
                    ...
                    Task.Run(() =>
                    {
                        ...
                        if (payload.Length + 4 < imageBytes)
                        {
                            ...
                            imageResult.Save("secret.png", ImageFormat.Png); //($"secret.{extension}");
                        }
                        else
                        {
